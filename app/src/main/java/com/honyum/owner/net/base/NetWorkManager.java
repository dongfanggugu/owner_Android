...
public class NetWorkManager {
    ...
    private WeakReference<BaseActivity> bact = null;
...
//			}
    ...
    public boolean addTast(NetTask netTask, String title, String info) {
        ...
        this.bact = netTask.getWeakBase();
        if (checkNetConn(bact.get())) {

            if (netTask.isSyncRequest()) {

                if (null == mProgressDialog) {
                    mProgressDialog = new ProgressDialog(bact.get());
                    if (title != null) {
...
//            }
     ...
     * 请求任务对话框点击取消监听器
     */
    private static onDialogCancelListener onDialogCancelListener;

    public interface onDialogCancelListener {
        void onDialogCancelListener();
    }

    public static void setOnDialogCancelListener(NetWorkManager.onDialogCancelListener onDialogCancelListener) {
        NetWorkManager.onDialogCancelListener = onDialogCancelListener;
    }
...
//        msg.arg1 = -1;
//        msg.arg2 = -10;
//        msg.obj = "网络连接错误，请检查网络设置";
//        manager.handleResult(msg);
    ...
    public static boolean checkGpsIsOpen(Context context) {
        ...
        boolean GPS_isOpen = lm.isProviderEnabled(LocationManager.GPS_PROVIDER);

        Log.d("TAG", "GPS_isOpen==>" + GPS_isOpen);
    ...
    public void handleResult(Message msg) {
        if (bact != null && msg != null) {
            bact.get().getHandler().sendMessage(msg);
        }
    }
}
