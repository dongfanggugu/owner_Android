...
public abstract class NetTask extends Thread {
    ...
    protected WeakReference<BaseActivity> mContexts = new WeakReference<BaseActivity>(
            null);
    ...
    public String mStrRequest = null;
    ...
    public NetTask(String serverAdd, String request) {
        SERVER_URL = serverAdd;
        mStrRequest = request;
    }
    ...
    protected void onFailed(NetTask task, String errorCode, String errorMsg) {
        ...
        BaseActivity context = mContexts.get();
        Message msg = Message.obtain();
        msg.arg1 = -1;
        msg.arg2 = Utils.getInt(errorCode);
        msg.obj = errorMsg;
//		context.getHandler().sendMessage(msg);
    ...
    public WeakReference<BaseActivity> getWeakBase() {
        return mContexts;
    }
     ...
     * @param baseAct
     */
    public void setBaseActivity(BaseActivity baseAct) {
        mContexts = new WeakReference<BaseActivity>(baseAct);
    }
    ...
    public void run() {
        ...
        if (!Utils.isEmpty(mStrRequest)) {
            uploadJson = mStrRequest;
        } else {
            uploadJson = getJson(mRequest);
        }
...
//            stopTask(msg);
            onFailed(this, "-5", "网络连接失败，请检查网络设置");
            return;
        }
...
//            stopTask(msg);
            onFailed(this, "-6", "服务器连接失败，请检查网络设置");
            return;
        }
     ...
     * @return
     */
    public static String getJson(RequestBean request) {
        try {
            return JacksonJsonUtil.beanToJson(request);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "";
